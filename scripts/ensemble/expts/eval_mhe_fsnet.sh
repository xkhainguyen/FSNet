...
for MHE_DIR in $(ls -d $P_NSC/202605*FSNet*_mhe5_seed* 2>/dev/null) ; do
  if [ ! -f "$MHE_DIR/model.pt" ] ; then
    echo "SKIP $MHE_DIR" ; continue
  fi
  RUN=$(basename $MHE_DIR)
  echo "######## $RUN ########"
  # FSNet eval is more expensive — focus on best post+best_merit and best_obj
