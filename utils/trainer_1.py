...
def load_instance(config):
    ...
    if prob_name not in problem_names:
        raise NotImplementedError(
            f"Problem type '{prob_type}_{prob_name}' not implemented")
    ...
    if config['ablation'] == True:
        result_save_dir = os.path.join('ablation_results', prob_type, prob_name, str(
            data), config['network'] + '_' + config['method'], 'dist_' + str(config['FSNet']['dist_weight']) + '_diff_' + str(config['FSNet']['max_diff_iter']))
    else:
        timestamp = time.strftime("%Y%m%d-%H%M%S")
        result_save_dir = os.path.join('results', prob_type, prob_name, str(
            data), timestamp + '_' + config['network'] + '_' + config['method'] + '_seed' + str(seed) + '_dropout' + str(config['dropout']))
...
def create_model(data, method, config):
    ...
    if network == 'MLP':
        if method == "DC3" or method == "sup_partial":
            ...
            model = MLP(data.xdim, hidden_dim, out_dim,
                        num_layers=num_layers, dropout=dropout)
        else:
            model = MLP(data.xdim, hidden_dim, data.ydim,
                        num_layers=num_layers, dropout=dropout)
    else:
...
class Trainer:
    ...
    def _penalty_loss(self, X_batch: torch.Tensor, Y_pred_scaled: torch.Tensor, metrics: Dict) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        eq_violation = self.data.eq_resid(
            X_batch, Y_pred_scaled).square().sum(dim=1)
        ineq_violation = self.data.ineq_resid(
            X_batch, Y_pred_scaled).square().sum(dim=1)

        eq_violation_l1 = self.data.eq_resid(
            X_batch, Y_pred_scaled).abs().sum(dim=1)
        ineq_violation_l1 = self.data.ineq_resid(
            X_batch, Y_pred_scaled).abs().sum(dim=1)
    ...
    def _adaptive_penalty_loss(self, X_batch: torch.Tensor, Y_pred_scaled: torch.Tensor, metrics: Dict) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        with torch.no_grad():
            self.adaptive_eq_weight = torch.clamp(
                self.adaptive_eq_weight + self.config_method['increasing_rate'] * eq_violation.mean(), min=0.0, max=self.config_method['eq_pen_weight_max'])
            self.adtaptive_ineq_weight = torch.clamp(
                self.adaptive_ineq_weight + self.config_method['increasing_rate'] * ineq_violation.mean(), min=0.0, max=self.config_method['ineq_pen_weight_max'])
    ...
    def _fsnet_loss(self, X_batch: torch.Tensor, Y_pred_scaled: torch.Tensor, metrics: Dict) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        pre_eq_violation = self.data.eq_resid(
            X_batch, Y_pred_scaled).square().sum(dim=1)
        pre_ineq_violation = self.data.ineq_resid(
            X_batch, Y_pred_scaled).square().sum(dim=1)
    ...
    def _sup_partial_loss(self, X_batch: torch.Tensor, Y_pred_scaled: torch.Tensor, Y_true: torch.Tensor, metrics: Dict, epoch_metrics: Dict) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        loss = huber(
            Y_final - Y_true[:, self.data.partial_vars]).mean(dim=1)  # [B]
    ...
    def _sup_pen_loss(self, X_batch: torch.Tensor, Y_pred_scaled: torch.Tensor, Y_true: torch.Tensor, metrics: Dict, epoch_metrics: Dict) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        loss_ineq_term = self.config_method['ineq_pen_weight'] * \
            pre_ineq_violation
    ...
    def _semi_loss(self, X_batch, Y_pred_scaled, Y_true, metrics, epoch_metrics):
        ...
        idx_unsup = torch.tensor(
            [i for i in range(B) if i not in idx_sup], device=X_batch.device)
        ...
        loss_sup[idx_sup] = sup_weight * \
            huber(Y_final[idx_sup] - Y_true[idx_sup]).mean(dim=1)
        ...
        # only apply unsupervised terms to unlabeled subset
        loss_unsup[idx_sup] = 0.0
    ...
    def train_epoch(self, train_loader: DataLoader, epoch: int) -> Dict[str, float]:
        ...
        for batch_idx, (X_batch, Y_true) in enumerate(train_loader):
            ...
            loss, batch_metrics = self.compute_batch_loss(
                X_batch, Y_pred, Y_true, epoch_metrics)
    ...
    def train(self):
        ...
        if self.config['checkpoint']:
            print(
                f"Loading model from checkpoint: {self.config['checkpoint']}")
            model_save_content = torch.load(
                self.config['checkpoint'],
                map_location=DEVICE,
                weights_only=False
            )
            # Ensure dropout is set correctly
            model_save_content['config']['dropout'] = self.config['dropout']
            self.model = create_model(
                self.data, self.method, model_save_content['config'])
        ...
        if self.config['checkpoint']:
            ...
            decay = optim.lr_scheduler.StepLR(self.optimizer, step_size=self.config.get(
                'lr_decay_step', 50), gamma=self.config.get('lr_decay', 0.9))
    ...
    def _save_model_and_results(self, train_history, val_history,
        ...
        print(
            f"  - {results_filename} (training history, metrics, detailed test results)")
